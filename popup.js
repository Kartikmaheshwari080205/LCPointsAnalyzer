const CATEGORIES = ["Daily Challenge", "Login", "Contest", "Monthly Challenge", "Contribution", "Survey", "Others"];
const COLORS = ["#4caf50", "#2196f3", "#ff9800", "#9c27b0", "#f44336", "#607d8b", "#795548"];
...
function aggregate(points) {
  ...
  points.forEach((item) => {
    ...
    const cat = item.category || categorize(desc);
...
function drawPie(totals) {
  ...
  const legendHTML = entries.map(([category, points], index) => {
    const sliceAngle = (points / total) * 2 * Math.PI;
    ctx.beginPath();
...
async function refreshDisplay() {
  ...
  for (const cat of CATEGORIES) {
    ...
    if (catPoints > 0) {
      hasData = true;
      const percentage = ((catPoints / totalPoints) * 100).toFixed(1);
      tableHTML += `
        <tr>
